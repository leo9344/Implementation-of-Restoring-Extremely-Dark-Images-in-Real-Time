...
class Net(nn.Module):
    ...
    def downshuffle(self,var,r):
        b,c,h,w = var.size()
        out_channel = c*(r**2) # b c h w
        out_h = h//r 
        out_w = w//r       # b c out_h, r our_w r ->  b c r r out_h out_w
        return var.contiguous().view(b, c, out_h, r, out_w,
             r).permute(0,1,3,5,2,4).contiguous().view(b,out_channel, out_h, out_w).contiguous()
